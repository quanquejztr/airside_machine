...
| Status checks required | `build` (from `.github/workflows/python-app.yml`) |
...
## 7. What GitHub cannot enforce

These stay **human / review** rules (documented in COLLABORATION_RULES.md):
...
- [ ] Protect `main` (PR required + CI must pass)
...
<your-initials>/<topic>, run pytest before pushing, open PR using
